...
spec = do
   ...
   describe "Data.Matrix.SymmetryOperationsSymbols.Parse.parseSymmetryOperation" $ do
     ...
     it "parse ' -2 0,0,0; 0,0,0' is just something" $ do
       parseSymmetryOperation " -2- 0,0,0; 0,0,0" `shouldBe` Just ("-2","-","0,0,0","0,0,0")

     it "parse ' -2- 0,0,0; 0,0,0' is nothing" $ do
       parseSymmetryOperation " -2 0,0,0; 0,0,0" `shouldBe` Nothing

     it "parse ' -2 x,y,z; 0,0,0' is nothing" $ do
